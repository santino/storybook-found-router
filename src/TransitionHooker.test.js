...
describe('TransitionHooker', () => {
  ...
  const wrap = () => mount(<TransitionHooker {...routerProp} story={storyFn} />)

  describe('React Component', () => {
    afterEach(() => {
      routerProp.router.addTransitionHook.mockClear()
    })
    ...
    it('adds the internal onTransition method as a TransitionHook', () => {
      const wrapper = wrap()
      wrapper.instance().removeTransitionHook = jest.fn()

      expect(routerProp.router.addTransitionHook).toHaveBeenCalledTimes(1)
      expect(routerProp.router.addTransitionHook).toHaveBeenCalledWith(
        wrapper.instance().onTransition
      )
    })

    it('sets and execute removeTransitionHook method on componentWillUnmount', () => {
      const wrapper = wrap()
      const mockRemoveTransitionHook = jest.fn()
      wrapper.instance().removeTransitionHook = mockRemoveTransitionHook

      expect(wrapper.instance().componentWillUnmount).toBeDefined()
      wrapper.unmount()
      expect(mockRemoveTransitionHook).toHaveBeenCalled()
    })
  })
